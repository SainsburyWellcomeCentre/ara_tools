function [out,atlas] = cutAtlas(atlas, dim, plane, varargin)
%% Cut Allen atlas annotation along a given dimension
%
% function out = aratools.projectAtlas.cutAtlas(atlas, dim, plane, 'PARAM1','VAL1')
% 
% Inputs
%   atlas   - A 3D matrix of area labels
%   dim     - [1 by default] dimension to projected along. 1 for top-down view.
%   plane   - [middle by default] the plane at which the cut happens
%
% Inputs [param/val]
...
for ii=1:length(groupChildren)
    ...
    for jj=1:length(groupingStruct(ii).children)
        f=find(cutAtlas==groupingStruct(ii).children(jj));
        if length(f)>1
            cutAtlas(f)=groupingStruct(ii).ARAindex;
        end
    end
end
...
for ii=1:length(groupIndsInProjection)
    ...
    for ii=2:length(inds)
        cutAtlas(cutAtlas==inds(ii))=inds(1);
    end
end


out.projectedAtlasRaw=cutAtlas; %create output structure
out.projectedAtlas=cutAtlas;
out.dim=dim;
...
for ii=1:length(N)
    ...
    if (SL.depth(indInTable)<7 || ~contains(SL.name(indInTable), ', layer')) || ~groupLayers % groupLayers blocks layer grouping from command line argument 
